...
export default function EditIndustryPage() {
  ...
  const industryId = parseInt(params.id as string);
  ...
  useEffect(() => {
    const fetchIndustry = async () => {
      try {
        ...
        setError("Failed to load industry. Please try again.");
      } finally {
        setIsLoading(false);
      }
    };

    if (isAuthenticated && industryId) {
      fetchIndustry();
    }
  }, [isAuthenticated, industryId]);
  ...
  const handleLogoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setLogoFile(null);
      setLogoPreview(industry?.logo ?? null);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="min-w-0 space-y-6">
        <p className="text-sm text-slate-500">Loading industry...</p>
      </div>
    );
  }
